# Scrapers Branch Sessions

Session log for the **`scrapers` lab branch**. Renamed from `SCRAPER_BRANCH_SESSIONS.md` on
2026-09-03 ahead of the merge into `main`: the main project keeps its own
`SCRAPER_BRANCH_SESSIONS.md` for pipeline work, and this file carries the lab's history under a
name that cannot collide with it. The main project's session history was removed
from this worktree on purpose — it recorded feasibility decisions (single Sweden
source, no German collection) that this lab does not inherit — so from L0 onward
this log covers only scraper lab work.

Keep each increment in one session. Unfinished work moves to the next row rather
than expanding the active increment.

**Branch outcome (2026-09-03).** 8 collector lanes built (PL, CZ, Adzuna DE+NL,
FR, BE, NO, FI, DE-BA), 7 with a green DoD; Finland built but blocked on a KEHA
activation bound to a Finnish Y-tunnus. The German active lane is closed: BA
Jobsuche step 1 done, the segmented census (step 2a) **cancelled** by decision,
and the frozen NUTS-3 panel (step 2b) **done and accepted**. `de-nuts3-panel` is
the scope chosen for publication; `de-stock-segmented` and `de-all-window` are
archived, unpublished, never re-swept. Ongoing panel sweeps are delegated to the
main project — see `PANEL_HANDOVER.md` for the runbook, the publish evidence, the
known issues, the data-transfer archives, and the merge procedure. Gate at
handover: `make check` green, 439 tests.
...
| P6 | 2026-09-03 | **Complete** | **Merge preparation** (docs + data transfer, no collector code). (1) **Data carried into git:** `data/raw/` is gitignored by policy on both branches and this worktree is removed after the merge, so the three German BA scopes were archived to `data/handover/ba/` under one narrow `.gitignore` exception — `de-nuts3-panel.zip` (1.27 MB, 28,900 rows), `de-stock-segmented.zip` (9.08 MB, 203,674 rows), `de-all-window.zip` (0.49 MB, 10,000 rows); 10.8 MB total for 190 MB of NDJSON (~17×), with `sha256` digests and row counts in `CHECKSUMS.json`. Restore procedure and the rule that restoring the census does **not** make it re-sweepable: `PANEL_HANDOVER.md` §8. (2) **Merge made conflict-free:** the lab log was renamed `SESSIONS.md` → **`SCRAPER_BRANCH_SESSIONS.md`** and main's `SESSIONS.md` restored byte-identical onto this branch, removing the only overlapping file; all cross-references in `AGENTS.md`, `PANEL_HANDOVER.md`, `SCRAPER_FEASIBILITY.md`, `SCRAPER_ROADMAP.md`, `SCRAPER_SOURCE_LANDSCAPE.md` retargeted. `git merge-tree --write-tree main HEAD` now exits **0** with no conflicted paths. (3) **Merge guide** (`PANEL_HANDOVER.md` §9): branch-off-main procedure with `--no-commit --no-ff`, plus the three files that merge cleanly yet change main's behaviour and must be handled by hand — `Makefile` (lab version drops main's `evaluate`/`dbt`/`sample`/`site`/`release-check`/`sweep*` targets; must be **combined**, not taken from one side), `pyproject.toml`/`uv.lock` (union deps then relock), `SOURCE_FEASIBILITY.md` (deleted by the lab, live on main; keep main's copy). Post-merge verification: restore the panel archive, then `scripts/check_ba_panel_readiness.py` must still print six PASS lines and hash `545b162ec6e5fbdc…`. | `make check` green (439 tests); merge dry run clean (exit 0) |
## Session Notes
...
- Replaced the main-project `SCRAPER_BRANCH_SESSIONS.md` history with this lab log.
...
- **Housekeeping:** resolved a pre-existing in-progress merge of
  ...
  markers in the lab `Makefile` and `SCRAPER_BRANCH_SESSIONS.md` (both blocked `make check`).
...
- **Order followed.** SCRAPERS.md -> SCRAPER_ROADMAP.md -> SCRAPER_SOURCE_LANDSCAPE.md
  -> SCRAPER_FEASIBILITY.md -> SCRAPER_BRANCH_SESSIONS.md, then the feasibility gate **before**
